...
x                       = [0.04 0.03 0.05 0.07];    %[m] x postition as defined in figure
y                       = [-0.04 -0.02 0.02 0.05];  %[m] y postition as defined in figure
h                       = [0.05 0.06 0.07 0.09];    %[m] desired height
times                   = [0 4 6 10];               %[s] time at which the gripper needs to arrive at the position 

robot_to_conveyorbelt   = 0.05;                     %[m] distance from the robot to the conveyorbelt

%See Rutger's drawing
gripper_length          = 0;                        %[m] distance between the gripper attachment point and the robot arm
AB                      = 0.081;                    %[m]
BC                      = 0.165;                    %[m]
BG                      = 0.080;                    %[m]
CD                      = 0.130;                    %[m]
...
for i = 1:n_coordinates
    ...
    intermediate_coordinate(3)  = sqrt( coordinate(1) * coordinate(1) + intermediate_coordinate(2) * intermediate_coordinate(2) ); %Pythagorean theorem
    intermediate_coordinate(1)  = atan2( intermediate_coordinate(2), coordinate(1) ); %Standard arctangent rule
